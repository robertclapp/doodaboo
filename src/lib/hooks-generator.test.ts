...
describe("template content sanity", () => {
  ...
  it("'x-but-y' capitalizes the subject", () => {
    const v = generateHooks({ subject: "typescript" }).find(
      (x) => x.template.id === "x-but-y",
    );
    assert.ok(v);
    assert.match(v!.hook, /^Typescript,/);
  });

  it("'not-what-you-think' capitalizes the subject", () => {
    const v = generateHooks({ subject: "react hooks" }).find(
      (x) => x.template.id === "not-what-you-think",
    );
    assert.ok(v);
    assert.match(v!.hook, /^React hooks isn't what you think\./);
  });

  it("'in-x-seconds' capitalizes the subject", () => {
    const v = generateHooks({ subject: "tailwind" }).find(
      (x) => x.template.id === "in-x-seconds",
    );
    assert.ok(v);
    assert.match(v!.hook, /^Tailwind in 10 seconds\./);
  });

  it("'the-x-that' uses 'framework that changed everything'", () => {
    const v = generateHooks({ subject: "rust" }).find(
      (x) => x.template.id === "the-x-that",
    );
    assert.ok(v);
    assert.match(v!.hook, /rust framework that changed everything/);
  });

  it("'i-was-wrong' appends 'Here's the receipt.'", () => {
    const v = generateHooks({ subject: "remote work" }).find(
      (x) => x.template.id === "i-was-wrong",
    );
    assert.ok(v);
    assert.match(v!.hook, /^I was wrong about remote work\. Here's the receipt\./);
  });

  it("'would-you' embeds the word 'ship'", () => {
    const v = generateHooks({ subject: "microservices" }).find(
      (x) => x.template.id === "would-you",
    );
    assert.ok(v);
    assert.match(v!.hook, /ship/);
  });

  it("'rules' uses a number from NUMBERS and includes the subject", () => {
    const v = generateHooks({ subject: "debugging" }).find(
      (x) => x.template.id === "rules",
    );
    assert.ok(v);
    assert.match(v!.hook, /^\d+ rules for debugging/);
  });

  it("'before-after' appends '— same effort, very different result.'", () => {
    const v = generateHooks({ subject: "content strategy" }).find(
      (x) => x.template.id === "before-after",
    );
    assert.ok(v);
    assert.match(v!.hook, /Before\/after content strategy/);
    assert.match(v!.hook, /same effort/);
  });

  it("'list-n-things-2' mentions the number and 'I wish'", () => {
    const v = generateHooks({ subject: "css" }).find(
      (x) => x.template.id === "list-n-things-2",
    );
    assert.ok(v);
    assert.match(v!.hook, /css I wish I'd known sooner/);
  });

  it("'you-think' confronts with 'You don't.'", () => {
    const v = generateHooks({ subject: "system design" }).find(
      (x) => x.template.id === "you-think",
    );
    assert.ok(v);
    assert.match(v!.hook, /You think you understand system design\. You don't\./);
  });
});

// ── Additional variantsForPlatform coverage ─────────────────────────────

describe("variantsForPlatform — additional platforms", () => {
  it("facebook gets fitsAll and facebook-listed variants only", () => {
    const v = generateHooks({ subject: "growth" });
    const fb = variantsForPlatform(v, "facebook");
    assert.ok(fb.length > 0);
    for (const x of fb) {
      assert.ok(x.fits.includes("facebook"), `${x.id} not fit for facebook`);
    }
  });

  it("linkedin excludes tiktok-only templates like 'in-x-seconds'", () => {
    const v = generateHooks({ subject: "growth" });
    const li = variantsForPlatform(v, "linkedin");
    assert.ok(!li.find((x) => x.template.id === "in-x-seconds"));
  });

  it("shorts excludes linkedin-only templates like 'i-was-wrong'", () => {
    const v = generateHooks({ subject: "cloud" });
    const sh = variantsForPlatform(v, "shorts");
    // 'i-was-wrong' only fits x/threads/linkedin, NOT shorts
    assert.ok(!sh.find((x) => x.template.id === "i-was-wrong"));
  });

  it("threads gets 'everyone-wrong' template", () => {
    const v = generateHooks({ subject: "productivity" });
    const th = variantsForPlatform(v, "threads");
    assert.ok(th.find((x) => x.template.id === "everyone-wrong"));
  });
});

// ── HOOK_FAMILIES structure ────────────────────────────────────────────────

describe("HOOK_FAMILIES — structure", () => {
  ...
  it("contains all expected family types", () => {
    const ids = new Set(HOOK_FAMILIES.map((f) => f.id));
    for (const expected of ["list", "contrarian", "personal", "question", "promise", "warning", "curiosity"]) {
      assert.ok(ids.has(expected as any), `missing family: ${expected}`);
    }
  });
});
