...
// ── Additional: generateHooks — number rotation (pick) ─────────────────────

describe("generateHooks — number rotation in list templates", () => {
...
// ── Additional: variantsForPlatform — empty result for unsupported platform ─

describe("variantsForPlatform — unsupported platform", () => {
  it("returns subsets: tiktok variants are a subset of 'all' variants", () => {
    const all = generateHooks({ subject: "crypto" });
    const tiktokVariants = variantsForPlatform(all, "tiktok");
    assert.ok(tiktokVariants.length <= all.length);
    // Every tiktok variant is present in the full list
    for (const v of tiktokVariants) {
      assert.ok(all.find((x) => x.id === v.id));
    }
  });
...
// ── Additional: generateHooks — 'x-but-y' capitalizes the subject ──────────

describe("generateHooks — capitalize behavior in 'x-but-y'", () => {
...
// ── Additional: HOOK_FAMILIES has 7 entries ─────────────────────────────────

describe("HOOK_FAMILIES completeness", () => {
  it("has exactly 7 families", () => {
    assert.equal(HOOK_FAMILIES.length, 7);
  });

  it("every family has a non-empty id and label", () => {
    for (const f of HOOK_FAMILIES) {
      assert.ok(f.id.length > 0, `family has empty id`);
      assert.ok(f.label.length > 0, `family ${f.id} has empty label`);
    }
  });
});
