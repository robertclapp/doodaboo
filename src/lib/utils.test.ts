...
// ── Additional coverage for cn (clsx wrapper) ──────────────────────────────
...
// ── Additional coverage for statusColor / priorityColor exact values ────────
...
// ── Additional coverage for slug edge cases ─────────────────────────────────

describe("slug — additional edge cases", () => {
  it("strips spaces between words before applying the 4-char cap", () => {
    // "AB CD" → strip spaces → "ABCD" → slice(0,4) = "ABCD"
    assert.equal(slug("AB CD"), "ABCD");
  });

  it("numeric-only input is preserved as uppercase digits", () => {
    assert.equal(slug("42"), "42");
  });

  it("mixed case and symbols become uppercase letters only", () => {
    assert.equal(slug("hello-world!"), "HELL");
  });
});

// ── Additional dueStatus — regression: 'soon' vs 'later' boundary ──────────

describe("dueStatus — soon/later boundary", () => {
  const now = Date.now();
