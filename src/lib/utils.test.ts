...
// cn() wraps clsx; we test it lightly to confirm the import works.
...
describe("initials — additional edge cases", () => {
  it("handles unicode characters in names", () => {
    // Just ensure no crash; initials of ascii-adjacent
    const r = initials("Ångela Öhgren");
    assert.ok(r.length > 0);
  });

  it("handles hyphenated names (splits on space only)", () => {
    // "Smith-Jones" is one word for initials purposes — only the first char is taken
    assert.equal(initials("Adam Smith-Jones"), "AS");
  });
});

describe("timeAgo — boundary edge cases", () => {
  it("exactly 60s falls into minutes bucket (60 rounds to 1m)", () => {
    const now = Date.now();
  ...
  it("exactly 0s is '0s'", () => {
    // Within the same millisecond; Math.round(0/1000)=0
...
describe("dueStatus — edge cases", () => {
  it("returns 'none' for an empty string", () => {
    // An empty string is falsy, same as undefined
    assert.equal(dueStatus("" as any), "none");
  });

  it("'today' boundary: midnight start-of-today is 'today'", () => {
    const startOfToday = new Date();
    startOfToday.setHours(0, 0, 0, 0);
    // Use the pin-clock 'now' at noon so we're clearly "today"
    const noon = new Date();
    noon.setHours(12, 0, 0, 0);
    assert.equal(dueStatus(startOfToday.toISOString(), noon.getTime()), "today");
  });
});

describe("slug — additional edge cases", () => {
  it("handles all-space input", () => {
    assert.equal(slug("   "), "");
  });

  it("handles input with only digits", () => {
    assert.equal(slug("1234"), "1234");
  });

  it("preserves up to 4 characters total", () => {
    assert.equal(slug("ABCDE"), "ABCD");
    assert.equal(slug("AB"), "AB");
  });
});

describe("priorityColor — spot checks", () => {
...
describe("statusColor — spot checks", () => {
