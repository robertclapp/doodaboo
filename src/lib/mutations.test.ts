...
// ── Additional: setTheme all values / immutability ─────────────────────────

describe("setTheme — all theme values", () => {
  for (const theme of ["light", "dark", "system"] as const) {
    it(`setTheme("${theme}") sets theme to "${theme}"`, () => {
      assert.equal(setTheme(fresh(), theme).theme, theme);
    });
  }

  it("setTheme does not mutate the original state", () => {
    const s0 = fresh();
    const original = s0.theme;
    setTheme(s0, "dark");
    assert.equal(s0.theme, original);
  });
});

// ── Additional: WORKSPACE_VERSION constant ────────────────────────────────
...
// ── Additional: moveTaskStatus edge cases ────────────────────────────────

describe("moveTaskStatus (additional)", () => {
  it("moveTaskStatus is a no-op for unknown taskId", () => {
    const s0 = fresh();
  ...
  it("moveTaskStatus updates the status to the target value", () => {
    const s0 = fresh();
...
// ── Additional: addUser immutability ─────────────────────────────────────

describe("addUser — immutability", () => {
  it("does not mutate the original users array", () => {
    const s0 = fresh();
    const originalLen = s0.users.length;
    addUser(s0, { name: "New", handle: "new", color: "#fff" });
    assert.equal(s0.users.length, originalLen);
  });
});

// ── Additional: createTask number assignment ──────────────────────────────

describe("createTask — task number assignment", () => {
  it("assigns task number equal to project.nextTaskNumber before increment", () => {
    const s0 = fresh();
    const project = s0.projects[0];
    const expectedNumber = project.nextTaskNumber;
    const r = createTask(s0, { projectId: project.id, title: "numbered" });
    assert.equal(r.task.number, expectedNumber);
  });

  it("sequential tasks get consecutive numbers", () => {
    const s0 = fresh();
    const project = s0.projects[0];
    const r1 = createTask(s0, { projectId: project.id, title: "first" });
    const r2 = createTask(r1.state, { projectId: project.id, title: "second" });
    assert.equal(r2.task.number, r1.task.number + 1);
  });
});
