...
describe("createProject — default values not previously covered", () => {
  it("default status is 'todo'", () => {
    const r = createProject(fresh(), { name: "X", key: "X" });
    assert.equal(r.project.status, "todo");
  });

  it("default priority is 'medium'", () => {
    const r = createProject(fresh(), { name: "X", key: "X" });
    assert.equal(r.project.priority, "medium");
  });

  it("default accent is '#ff5c1a'", () => {
    const r = createProject(fresh(), { name: "X", key: "X" });
    assert.equal(r.project.accent, "#ff5c1a");
  });

  it("uses provided accent over default", () => {
    const r = createProject(fresh(), { name: "X", key: "X", accent: "#123456" });
    assert.equal(r.project.accent, "#123456");
  });

  it("targetDate is undefined by default", () => {
    const r = createProject(fresh(), { name: "X", key: "X" });
    assert.equal(r.project.targetDate, undefined);
  });
});

describe("removeSnapshot — no-op cases", () => {
  it("is a no-op for unknown snapshotId (no crash, no state change in other posts)", () => {
    const s0 = fresh();
    const post = s0.posts[0];
    // Add a snapshot so we can distinguish "not removing" from "no snapshots"
    const r1 = addSnapshot(s0, post.id, {
      atMinutes: 10,
      impressions: 100,
      views: 90,
    ...
    const countBefore = r1.state.posts.find((p) => p.id === post.id)!.snapshots.length;
    const next = removeSnapshot(r1.state, post.id, "snap_does_not_exist");
    const countAfter = next.posts.find((p) => p.id === post.id)!.snapshots.length;
    assert.equal(countAfter, countBefore);
  });

  it("is a no-op for unknown postId (other posts unchanged)", () => {
    const s0 = fresh();
    const next = removeSnapshot(s0, "po_does_not_exist", "snap_x");
    assert.equal(next.posts.length, s0.posts.length);
  });
});

describe("updatePost — no-op for unknown id", () => {
  it("returns unchanged state when post not found", () => {
    const s0 = fresh();
    const next = updatePost(s0, "po_nope", { title: "changed" });
    // State is structurally equivalent (posts array unchanged)
    assert.equal(next.posts.length, s0.posts.length);
    for (const p of next.posts) {
      assert.notEqual(p.title, "changed");
    }
  });
});

describe("deletePost — no-op for unknown id", () => {
  it("returns state with same post count when id is unknown", () => {
    const s0 = fresh();
    const next = deletePost(s0, "po_nope");
    assert.equal(next.posts.length, s0.posts.length);
  });
});

describe("addComment — unknown taskId", () => {
  it("returns the comment even when taskId does not match any task", () => {
    // addComment creates the Comment object regardless; the task map just
    // finds no matching task so the state is unchanged. The returned
    // comment is still populated.
    const s0 = fresh();
    const r = addComment(s0, "t_does_not_exist", "hello");
    // No matching task → comment is returned
    assert.ok(r.comment);
    assert.equal(r.comment!.body, "hello");
    // State has same task count
    assert.equal(r.state.tasks.length, s0.tasks.length);
  });

  it("uses ctx.actorId when provided", () => {
    const s0 = fresh();
    const task = s0.tasks[0];
    const r = addComment(s0, task.id, "actor test", { actorId: "u_actor" });
    assert.equal(r.comment!.authorId, "u_actor");
  });
});

describe("setTheme — all valid themes", () => {
  it("sets 'light'", () => {
    assert.equal(setTheme(fresh(), "light").theme, "light");
  });

  it("sets 'system'", () => {
    assert.equal(setTheme(fresh(), "system").theme, "system");
  });
});

describe("removeUser — filters from users array", () => {
  it("removes the user from the users array", () => {
    const s0 = fresh();
    const userId = s0.users[0].id;
    const next = removeUser(s0, userId);
    assert.ok(!next.users.find((u) => u.id === userId));
    assert.equal(next.users.length, s0.users.length - 1);
  });
});

describe("addLabel — new label is in state.labels", () => {
  it("appends the label to state.labels", () => {
    const s0 = fresh();
    const r = addLabel(s0, { name: "Bug", color: "#ff0000" });
    assert.ok(r.state.labels.find((l) => l.id === r.label.id));
    assert.equal(r.label.name, "Bug");
    assert.equal(r.label.color, "#ff0000");
  });
});
