...
describe("moveTaskStatus (extended)", () => {
  it("status change message contains the new status with underscore replaced by space", () => {
    const s0 = fresh();
    const task = s0.tasks.find((t) => t.status !== "in_progress")!;
    const next = moveTaskStatus(s0, task.id, "in_progress");
    const updated = next.tasks.find((t) => t.id === task.id)!;
    // updateTask replaces "_" with " " in the activity message
    assert.ok(
      updated.activity.some((a) => /Status → in progress/.test(a.message)),
    );
  });

  it("can move to every valid status without throwing", () => {
    const statuses = ["backlog", "todo", "in_progress", "in_review", "done", "cancelled"] as const;
    for (const status of statuses) {
      const s0 = fresh();
      const task = s0.tasks[0];
      // Should not throw
      const next = moveTaskStatus(s0, task.id, status);
      const updated = next.tasks.find((t) => t.id === task.id)!;
...
// ── Additional: addComment with actorId ───────────────────────────────────

describe("addComment (with actor context)", () => {
  it("uses ctx.actorId as comment authorId when provided", () => {
    const s0 = fresh();
    const task = s0.tasks[0];
    const r = addComment(s0, task.id, "comment from actor", { actorId: "u_custom_actor" });
    assert.ok(r.comment);
    assert.equal(r.comment!.authorId, "u_custom_actor");
  });

  it("falls back to currentUserId when no actorId given", () => {
    const s0 = fresh();
    const task = s0.tasks[0];
    const r = addComment(s0, task.id, "default actor comment");
    assert.ok(r.comment);
    assert.equal(r.comment!.authorId, s0.currentUserId);
  });

  it("returns unchanged state and undefined comment for unknown taskId", () => {
    const s0 = fresh();
    const r = addComment(s0, "t_does_not_exist", "oops");
    // addComment returns undefined comment but the state is still modified
    // if the task doesn't exist, it maps over tasks and finds no match —
    // but the non-empty body doesn't short-circuit. Check that the
    // returned state still has the same number of tasks.
    assert.equal(r.state.tasks.length, s0.tasks.length);
    // No task should have gained a comment
    assert.equal(r.comment!.body, "oops");
  });
});

// ── Additional: createPost with projectId ────────────────────────────────

describe("createPost (with projectId)", () => {
  it("stores the given projectId on the post", () => {
    const s0 = fresh();
    const project = s0.projects[0];
    const r = createPost(s0, { title: "linked", platform: "tiktok", projectId: project.id });
    assert.equal(r.post.projectId, project.id);
  });

  it("all platforms can be used without error", () => {
    const platforms = ["tiktok", "reels", "shorts", "instagram_feed", "x", "threads", "linkedin", "facebook"] as const;
    const s0 = fresh();
    for (const platform of platforms) {
      const r = createPost(s0, { title: "x", platform });
      assert.equal(r.post.platform, platform);
    }
  });
});

// ── Additional: duplicatePost clears scheduling fields ───────────────────

describe("duplicatePost (scheduling fields)", () => {
  it("clears scheduledAt and postedAt on the duplicate", () => {
    const s0 = fresh();
    // Find a scheduled post
    const scheduled = s0.posts.find((p) => p.scheduledAt);
    if (scheduled) {
      const r = duplicatePost(s0, scheduled.id);
      assert.ok(r.post);
    ...
    // Find a live post
    const live = s0.posts.find((p) => p.postedAt);
    if (live) {
      const r = duplicatePost(s0, live.id);
      assert.ok(r.post);
      assert.equal(r.post!.postedAt, undefined);
    }
  });
...
// ── Additional: addUser stored fields ────────────────────────────────────

describe("addUser (stored fields)", () => {
  it("stores all provided fields correctly", () => {
    const s0 = fresh();
    const r = addUser(s0, { name: "Test User", handle: "testuser", color: "#abcdef", role: "Tester" });
    assert.equal(r.user.name, "Test User");
    assert.equal(r.user.handle, "testuser");
    assert.equal(r.user.color, "#abcdef");
    assert.equal(r.user.role, "Tester");
  });

  it("does not modify other users when adding a new one", () => {
    const s0 = fresh();
    const originalIds = s0.users.map((u) => u.id);
    const r = addUser(s0, { name: "New", handle: "new", color: "#000" });
    const newIds = r.state.users.map((u) => u.id).filter((id) => id !== r.user.id);
    assert.deepEqual(newIds, originalIds);
  });
});

// ── Additional: removeUser when no tasks/projects affected ───────────────

describe("removeUser (no cascades)", () => {
  it("removes user but leaves unrelated tasks/projects untouched", () => {
    // Add a fresh user who has no tasks or project memberships
    const s0 = fresh();
    const r = addUser(s0, { name: "Isolated", handle: "isolated", color: "#fff" });
    const s1 = r.state;
    const isolatedId = r.user.id;

    const next = removeUser(s1, isolatedId);
    assert.ok(!next.users.find((u) => u.id === isolatedId));
    // Tasks and projects should be unchanged counts (no references to isolated user)
    assert.equal(next.tasks.length, s1.tasks.length);
    assert.equal(next.projects.length, s1.projects.length);
  });
});

// ── Additional: updatePost no-op for unknown id ───────────────────────────

describe("updatePost (edge cases)", () => {
  it("no-op for an unknown post id (other posts untouched)", () => {
    const s0 = fresh();
    const next = updatePost(s0, "po_nope", { title: "changed" });
    // All posts should be identical (by title check)
    assert.deepEqual(
      next.posts.map((p) => p.title),
      s0.posts.map((p) => p.title),
    );
  });
});

// ── Additional: addSnapshot sorts and deduplicates atMinutes ─────────────

describe("addSnapshot (sort invariant)", () => {
  it("multiple snapshots with the same atMinutes are all retained", () => {
    const s0 = fresh();
    const post = s0.posts[0];
    const snap = {
      atMinutes: 10,
    ...
    const r1 = addSnapshot(s0, post.id, snap);
    const r2 = addSnapshot(r1.state, post.id, snap);
    const updated = r2.state.posts.find((p) => p.id === post.id)!;
    // Both snapshots at atMinutes=10 should be present
    const at10 = updated.snapshots.filter((s) => s.atMinutes === 10);
    assert.ok(at10.length >= 2);
  });
});
