...
// ── Additional: vaultPaths structure ────────────────────────────────────────
...
// ── Additional: vaultExists false for uninitialized directory ───────────────
...
// ── Additional: initVault refuses overwrite without force ───────────────────

describe("initVault — overwrite guard", () => {
...
// ── Additional: migrate — version=undefined coerced to version 0 ────────────

describe("migrate — version coercion", () => {
  it("treats missing version as 0 and bumps to WORKSPACE_VERSION", () => {
    // version=undefined should be treated like version=0 (old/missing schema)
    const s = migrate({});
    assert.equal(s.version, WORKSPACE_VERSION);
  });

  it("preserves existing theme when it is a valid string", () => {
    const s = migrate({ version: 1, theme: "dark" });
    assert.equal(s.theme, "dark");
  });

  it("backfills theme to 'system' when theme is a non-string (number)", () => {
    // migrate checks `typeof obj.theme !== "string"` — non-strings get defaulted
    const s = migrate({ version: 1, theme: 42 });
    assert.equal(s.theme, "system");
  });

  it("backfills post.snapshots when missing from post", () => {
    const s = migrate({
      version: 1,
      posts: [{ id: "po_x", title: "t", platform: "tiktok" }],
    });
    // snapshots are always backfilled to []
    assert.deepEqual(s.posts[0].snapshots, []);
  });
});
