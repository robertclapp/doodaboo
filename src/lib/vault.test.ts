...
describe("defaultVaultRoot", () => {
  it("uses DOODABOO_VAULT env var when set", () => {
    ...
    process.env.DOODABOO_VAULT = "/tmp/custom-vault";
    try {
      assert.equal(defaultVaultRoot(), "/tmp/custom-vault");
    } finally {
  ...
  it("falls back to ~/.doodaboo when DOODABOO_VAULT is unset", () => {
    ...
    try {
      ...
      assert.equal(root, path.join(os.homedir(), ".doodaboo"));
    } finally {
...
describe("initVault — duplicate guard", () => {
  it("throws with 'already exists' when vault exists and force is false", async () => {
    const root = await tmpVault();
    await initVault(root, { force: true });
  ...
  it("succeeds with force=true on an existing vault", async () => {
    const root = await tmpVault();
    await initVault(root, { force: true });
    const paths = await initVault(root, { force: true });
    assert.ok((await fs.stat(paths.workspaceFile)).isFile());
  });
});

describe("withWorkspace — error propagation", () => {
  it("propagates errors thrown by the mutator", async () => {
    ...
    // Vault should still be readable after a failed mutation.
    const state = await loadWorkspace(root);
    assert.ok(state.version === WORKSPACE_VERSION);
  });
});
...
describe("saveWorkspace — backup pruning", () => {
  it("prunes backups beyond MAX_BACKUPS=20", async () => {
    ...
    // Create 25 saves to exceed the 20-backup ceiling.
    for (let i = 0; i < 25; i++) {
      // Stagger timestamps so filenames differ.
