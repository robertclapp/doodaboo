...
describe("migrate (trust boundary)", () => {
  ...
  it("preserves a valid existing currentUserId", () => {
    const s = migrate({
      version: 1,
      users: [
        { id: "u_a", name: "A", handle: "a", color: "#000" },
        { id: "u_b", name: "B", handle: "b", color: "#fff" },
      ],
      currentUserId: "u_b",
    });
    assert.equal(s.currentUserId, "u_b");
  });

  it("falls back currentUserId to 'u_unknown' when users is empty", () => {
    const s = migrate({ version: 1, users: [] });
    assert.equal(s.currentUserId, "u_unknown");
  });

  it("backfills hashtags when content exists but hashtags is missing", () => {
    const s = migrate({
      version: 1,
      posts: [
        {
  ...
  it("backfills memberIds on projects missing that field", () => {
    const s = migrate({
      version: 1,
      projects: [{ id: "p_x", key: "X", name: "X" }],
    });
  ...
  it("accepts version 1 (current) without throwing", () => {
    const s = migrate({ version: 1 });
    assert.equal(s.version, WORKSPACE_VERSION);
  });
});

// ── VaultNotFoundError / VaultCorruptError ────────────────────────────────

describe("VaultNotFoundError and VaultCorruptError", () => {
  it("VaultNotFoundError carries the path and correct name", () => {
    const err = new VaultNotFoundError("/some/path/workspace.json");
    assert.equal(err.name, "VaultNotFoundError");
    assert.equal(err.path, "/some/path/workspace.json");
    assert.ok(err instanceof Error);
    assert.match(err.message, /\/some\/path/);
  });

  it("VaultCorruptError has the correct name", () => {
...
describe("vaultPaths", () => {
  it("returns paths nested under the root", () => {
    const root = "/tmp/test-vault";
    const p = vaultPaths(root);
    assert.equal(p.root, root);
    assert.ok(p.workspaceFile.startsWith(root));
    assert.ok(p.backupsDir.startsWith(root));
    assert.ok(p.pluginsDir.startsWith(root));
    assert.ok(p.exportsDir.startsWith(root));
  });

  it("workspaceFile ends with workspace.json", () => {
    const p = vaultPaths("/tmp/v");
    assert.ok(p.workspaceFile.endsWith("workspace.json"));
  });
});
