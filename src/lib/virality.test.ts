...
// ── recommend with extraFactors ───────────────────────────────────────────

describe("recommend — extraFactors plumbing", () => {
  it("does not recommend improvement on an injected perfect-raw plugin factor", () => {
    // A plugin factor with raw=1.0 should be filtered out (already strong).
    const extra = {
      id: "plugin_factor",
      label: "Plugin factor",
      group: "content" as const,
    ...
    const recs = recommend(makePost(), 10, [extra]);
    assert.ok(!recs.find((r) => r.factorId === "plugin_factor"));
  });

  it("recommends improvement on an injected low-raw plugin factor", () => {
    const extra = {
      id: "plugin_low",
      label: "Plugin low",
      group: "content" as const,
    ...
    // Use a post that's already strong so the plugin is the main weakness.
    const recs = recommend(
      makePost({
        content: {
          hook: "Stop scrolling — this is the one tip you need.",
    ...
    assert.ok(recs.find((r) => r.factorId === "plugin_low"));
  });
});

// ── scoreLive zero-likes commentDepth branch ──────────────────────────────

describe("scoreLive — zero-likes commentDepth branch", () => {
  it("uses comments/50 path when likes=0, stays finite", () => {
    // commentDepth formula: if likes > 0 → comments/likes*5, else comments/50.
    const post = makePost({
      snapshots: [
        {
          id: "s1",
          ...
          impressions: 1000,
          views: 900,
          likes: 0, // triggers the /50 branch
          comments: 100,
          shares: 10,
          saves: 5,
          retentionPct: 40,
    ...
    const live = scoreLive(post)!;
    assert.ok(live, "scoreLive must return a value");
    assert.ok(Number.isFinite(live.value));
    assert.ok(live.factors.find((f) => f.id === "commentDepth"));
  });

  it("high comment-count with zero likes raises commentDepth toward 1", () => {
    const manyComments = makePost({
      snapshots: [
        {
          id: "s1",
          atMinutes: 20,
    ...
    const live = scoreLive(manyComments)!;
    const cdFactor = live.factors.find((f) => f.id === "commentDepth");
    assert.ok(cdFactor, "commentDepth factor must be present");
    assert.ok(cdFactor!.raw > 0, `commentDepth raw should be > 0, got ${cdFactor!.raw}`);
  });
});
