...
// ── Additional: scoreIntrinsic on all 8 platforms ─────────────────────────

describe("scoreIntrinsic — all platforms produce finite values", () => {
  const platforms: Platform[] = [
    "tiktok", "reels", "shorts", "instagram_feed",
    "x", "threads", "linkedin", "facebook",
  ];

  it("computedAt is a valid ISO string", () => {
    const s = scoreIntrinsic(makePost());
    assert.ok(!isNaN(Date.parse(s.computedAt)));
  });

  it("every factor has raw in [0,1]", () => {
    for (const platform of platforms) {
      const s = scoreIntrinsic(makePost({ platform }));
      for (const f of s.factors) {
        assert.ok(
          f.raw >= 0 && f.raw <= 1,
          `factor ${f.id} on ${platform} has raw=${f.raw}`,
        );
      }
    }
  });

  it("every factor has a non-empty hint", () => {
    for (const platform of platforms) {
      const s = scoreIntrinsic(makePost({ platform }));
      for (const f of s.factors) {
        assert.ok(f.hint.length > 0, `empty hint for ${f.id} on ${platform}`);
      }
    }
  });

  it("factor groups are all valid", () => {
    const validGroups = new Set(["content", "context", "traction", "diffusion"]);
    for (const platform of platforms) {
      const s = scoreIntrinsic(makePost({ platform }));
      for (const f of s.factors) {
        assert.ok(validGroups.has(f.group), `invalid group '${f.group}' for ${f.id}`);
      }
    }
  });
});

// ── Additional: scoreLive on all platforms ────────────────────────────────

describe("scoreLive — all platforms produce valid results", () => {
  const platforms: Platform[] = [
    "tiktok", "reels", "shorts", "instagram_feed",
    "x", "threads", "linkedin", "facebook",
  ];
  ...
  for (const platform of platforms) {
    it(`${platform} scoreLive stays in 0..100`, () => {
      const post = makePost({ platform, snapshots: [snap] });
      const live = scoreLive(post)!;
      assert.ok(live);
      assert.ok(live.value >= 0 && live.value <= 100, `live.value=${live.value} on ${platform}`);
      assert.ok(Number.isFinite(live.value));
    });
  }
});

// ── Additional: recommend returns stable potentialGain values ─────────────

describe("recommend — potentialGain is always positive", () => {
  it("all recommendations have potentialGain > 0", () => {
    const post = makePost({
      content: {
        hook: "",
        caption: "",
    ...
    const recs = recommend(post, 10);
    for (const r of recs) {
      assert.ok(r.potentialGain > 0, `${r.factorId} has potentialGain=${r.potentialGain}`);
    }
  });
});

// ── Additional: platformProfile structure ─────────────────────────────────

describe("platformProfile — additional structure checks", () => {
  it("all platforms have formatPreference for all 5 formats", () => {
    const formats: any[] = ["video", "image", "carousel", "text", "live"];
    ...
    for (const platform of platformList) {
      ...
      for (const fmt of formats) {
        const v = p.formatPreference[fmt];
        assert.ok(
          typeof v === "number" && v >= 0 && v <= 1,
          `${platform}.formatPreference.${fmt} = ${v}`,
        );
      }
    }
  });

  it("all platforms have peakDays in [0..6]", () => {
    const platformList: Platform[] = [
      "tiktok", "reels", "shorts", "instagram_feed",
      "x", "threads", "linkedin", "facebook",
    ];
    for (const platform of platformList) {
      const p = platformProfile(platform);
      for (const d of p.peakDays) {
        assert.ok(d >= 0 && d <= 6, `${platform} has invalid peakDay=${d}`);
      }
    }
  });

  it("all platforms have peakHours in [0..23]", () => {
    const platformList: Platform[] = [
      "tiktok", "reels", "shorts", "instagram_feed",
      "x", "threads", "linkedin", "facebook",
    ];
    for (const platform of platformList) {
      const p = platformProfile(platform);
      for (const h of p.peakHours) {
        assert.ok(h >= 0 && h <= 23, `${platform} has invalid peakHour=${h}`);
      }
    }
  });
});

// ── Additional: projectThreshold returns threshold field correctly ────────

describe("projectThreshold — threshold field", () => {
  it("returns the threshold value from the post", () => {
    const post = makePost({
      threshold: { metric: "views", value: 250_000, window: "7d" },
      snapshots: [
        {
          id: "s1",
          atMinutes: 30,
    ...
    const p = projectThreshold(post)!;
    assert.equal(p.threshold, 250_000);
  });

  it("projected is always > 0 when views > 0", () => {
    const post = makePost({
      threshold: { metric: "views", value: 100_000, window: "7d" },
      snapshots: [
        {
