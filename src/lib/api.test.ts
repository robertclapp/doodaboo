...
// ── readWorkspace / mutateWorkspace ────────────────────────────────────────

describe("readWorkspace", () => {
  let prevVaultRw: string | undefined;
  let vaultRootRw: string;

  beforeEach(async () => {
    vaultRootRw = await fs.mkdtemp(path.join(os.tmpdir(), "doodaboo-rw-"));
    await initVault(vaultRootRw, { force: true });
    prevVaultRw = process.env.DOODABOO_VAULT;
    process.env.DOODABOO_VAULT = vaultRootRw;
  });

  afterEach(async () => {
    if (prevVaultRw === undefined) delete process.env.DOODABOO_VAULT;
    else process.env.DOODABOO_VAULT = prevVaultRw;
    await fs.rm(vaultRootRw, { recursive: true, force: true });
  });

  it("returns the current workspace state from disk", async () => {
  ...
  it("reflects mutations persisted by mutateWorkspace", async () => {
    const before = await readWorkspace();
    const projectId = before.projects[0].id;

    await mutateWorkspace((s) => {
      const r = createTask(s, { projectId, title: "rw-test-task" });
      return { state: r.state, result: r.task.id };
    });

    const after = await readWorkspace();
    assert.ok(after.tasks.some((t) => t.title === "rw-test-task"));
  });
});

describe("mutateWorkspace", () => {
  let prevVaultMw: string | undefined;
  let vaultRootMw: string;

  beforeEach(async () => {
    vaultRootMw = await fs.mkdtemp(path.join(os.tmpdir(), "doodaboo-mw-"));
    await initVault(vaultRootMw, { force: true });
    prevVaultMw = process.env.DOODABOO_VAULT;
    process.env.DOODABOO_VAULT = vaultRootMw;
  });

  afterEach(async () => {
    if (prevVaultMw === undefined) delete process.env.DOODABOO_VAULT;
    else process.env.DOODABOO_VAULT = prevVaultMw;
    await fs.rm(vaultRootMw, { recursive: true, force: true });
  });

  it("returns the result value from the mutator callback", async () => {
    const before = await readWorkspace();
    const projectId = before.projects[0].id;
    const taskId = await mutateWorkspace((s) => {
      const r = createTask(s, { projectId, title: "mw-result-task" });
      return { state: r.state, result: r.task.id };
    });
    assert.match(taskId, /^t_/);
  });

  it("persists changes across sequential mutations", async () => {
    const before = await readWorkspace();
    const projectId = before.projects[0].id;

    const id1 = await mutateWorkspace((s) => {
      const r = createTask(s, { projectId, title: "first" });
      return { state: r.state, result: r.task.id };
    });
    const id2 = await mutateWorkspace((s) => {
      const r = createTask(s, { projectId, title: "second" });
      return { state: r.state, result: r.task.id };
    });

    const state = await readWorkspace();
    assert.ok(state.tasks.some((t) => t.id === id1));
    assert.ok(state.tasks.some((t) => t.id === id2));
  });
});
