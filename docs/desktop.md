...
`tauri dev` runs against) and the web build are unchanged.
...
gitignored and regenerated by `init`. Commit it only once it needs hand
edits — release signing config, extra manifest permissions — and commit
`src-tauri/gen/schemas` alongside.
